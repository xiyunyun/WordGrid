 ...
 *
 * 默认只读模式：只能查看和框选文字。
 * 点击「编辑」按钮切换到编辑模式，同时解锁：
 * - 单词拼写（头部 h3 → input）
 * - 词意（头部 p → input）
 * - 笔记正文（textarea）
 * 词性保持只读（用户明确不需要修改）。
 * 点击「完成」按钮保存所有改动并退出编辑模式（自动保存，无需手动保存按钮）。
 * 关闭弹窗时若处于编辑模式，自动保存草稿。
 *
 * 全局同步：调用 store.updateWord，由 store 内部的 syncEnabled 机制自动推送到 Supabase。
 *
 * 宽高根据内容自适应：
 * - 高度根据回车换行数决定
...
export default function NoteModal({ open, onClose, word }: NoteModalProps) {
  ...
  // 编辑时调用 updateWord 后，store 更新，此处能拿到最新值
  const latestWord = useWordStore((s) =>
    word ? s.words.find((w) => w.id === word.id) ?? word : null,
  );

  const note = latestWord?.note || "";
  const wordText = latestWord?.word || "";
  const meaning = latestWord?.meaning || "";

  // 进入编辑模式时，用最新值初始化三个草稿
  const enterEdit = () => {
    setDraftNote(note);
    setDraftWord(wordText);
    setDraftMeaning(meaning);
    setEditing(true);
    // 等待 textarea 渲染后聚焦
    setTimeout(() => {
  ...
  // 退出编辑模式并保存：比较三个字段，有变化才调用 updateWord
  const exitEdit = () => {
    if (latestWord) {
      const patch: Partial<Word> = {};
      const trimmedWord = draftWord.trim();
      const trimmedMeaning = draftMeaning.trim();
      if (trimmedWord && trimmedWord !== wordText) patch.word = trimmedWord;
      if (trimmedMeaning !== meaning) patch.meaning = trimmedMeaning;
      if (draftNote !== note) patch.note = draftNote;
      if (Object.keys(patch).length > 0) {
        updateWord(latestWord.id, patch);
      }
    }
    setEditing(false);
  };
  ...
  }, [open, editing, draftNote, draftWord, draftMeaning, note, wordText, meaning, latestWord]);
  ...
  // 按回车切分行（笔记宽度计算用显示中的内容）
  const displayNote = editing ? draftNote : note;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center animate-fade-in" data-modal="true">
      ...
      >
        {/* 头部 */}
        <header className="border-b border-ink/15 bg-paper-card px-5 py-3">
            ...
            {editing ? (
              <>
                ...
                <input
                  ...
                  placeholder="单词拼写"
                  className="w-full rounded-md border border-accent-gold/30 bg-paper-warm/30 px-2 py-1 font-serif text-xl font-medium leading-tight tracking-word text-ink focus:border-accent-gold focus:outline-none focus:ring-1 focus:ring-accent-gold/20"
                />
                {/* 词性只读显示 */}
                {latestWord.pos && (
                ...
                <input
                  ...
                  placeholder="释义"
                  className="w-full rounded-md border border-accent-gold/30 bg-paper-warm/30 px-2 py-1 font-body text-sm leading-snug text-ink-soft focus:border-accent-gold focus:outline-none focus:ring-1 focus:ring-accent-gold/20"
                />
              </>
            ) : (
              <>
        ...
        <footer className="border-t border-ink/10 bg-paper-warm/40 px-5 py-2 font-mono text-2xs uppercase tracking-editorial text-ink-light">
          {editing ? (
            <span className="text-accent-gold">编辑中 · 可修改单词/词意/笔记 · 按 ESC 退出并保存</span>
          ) : (
            <>
